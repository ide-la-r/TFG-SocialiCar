...
<head>
    ...
    <title>SocialiCar - Edita tu coche</title>
...
<body class="d-flex flex-column min-vh-100">
    ...
    $matricula = $_SESSION["coche"]["matricula"];
    ...
    while ($fila = $resultado->fetch_assoc()) {
        ...
        // id_usuario=?, seguro=?, marca=?, modelo=?, anno_matriculacion=?, kilometros=?,
        ...
        $modelo = $fila["modelo"];
        $marca = $fila["marca"];
        $anno_matriculacion = $fila["anno_matriculacion"];
        $matricula = $fila["matricula"];
        $kilometros = $fila["kilometros"];
        $tipo_combustible = $fila["combustible"];
        $tipo = $fila["tipo"];
        $transmision = $fila["transmision"];
        $color = $fila["color"];
        $potencia = $fila["potencia"];
        $numero_puertas = $fila["puertas"];
        $plazas = $fila["plazas"];
...
<br>
    ...
    if (isset($matricula, $marca, $modelo, $precio, $anno_matriculacion, $kilometros, $direccion, $cp, $provincia, $ciudad, $tipo_combustible, $transmision, $tipo_aparcamiento, $tipo, $precio, $color, $plazas, $puertas, $potencia, $descripcion)) {
        // Actualizar coche
        $actualizarCoche = $_conexion->prepare("UPDATE coche SET
            id_usuario=?, seguro=?, marca=?, modelo=?, anno_matriculacion=?, kilometros=?,
            combustible=?, transmision=?, provincia=?, ciudad=?, codigo_postal=?, direccion=?,
            tipo_aparcamiento=?, ruta_img_coche=?, tipo=?, precio=?, descripcion=?, color=?, plazas=?,
            puertas=?, potencia=?
        ...
        $actualizarCoche->bind_param("sisssissssissssissiiis",
            $id_usuario, $seguro, $marca, $modelo, $anno_matriculacion, $kilometros,
            ...
            $puertas, $potencia, $matricula
        );

        if (!$actualizarCoche->execute()) {
            die('Error al actualizar coche: ' . $actualizarCoche->error);
        }

        // Actualizar extras
        $actualizarExtras = $_conexion->prepare("UPDATE extras_coche SET
        ...
        $actualizarExtras->bind_param("iiiiiiiiiiiiiiiiiiiis",
            $aire_acondicionado, $gps, $wifi, $sensores_aparcamiento, 
            $camara_trasera, $control_de_crucero, $asientos_calefactables, $bola_remolque, 
            $fijacion_isofix, $apple_carplay, $android_carplay, $baca, $portabicicletas,
            $portaequipajes, $portaesquis, $bluetooth, $cuatro_x_cuatro, $mascota, $fumar,
            $movilidad_reducida, $matricula
        );

        if (!$actualizarExtras->execute()) {
            die('Error al actualizar extras: ' . $actualizarExtras->error);
        }
        ...
        // if (isset($rutas_imagenes) && is_array($rutas_imagenes)) {
        //     foreach ($rutas_imagenes as $ruta) {
        //         // Si tienes un id de imagen, puedes hacer UPDATE, si no, normalmente se borran y se insertan de nuevo
        //         // Aquí ejemplo de borrado e inserción:
        //         // $_conexion->query("DELETE FROM imagen_coche WHERE id_coche = '$matricula'");
        //         $insertarImagen = $_conexion->prepare("INSERT INTO imagen_coche (id_coche, ruta_img_coche) VALUES (?, ?)");
        //         if (!$insertarImagen) {
        //             die('Error en prepare imagenes: ' . $_conexion->error);
        //         }
        //         $insertarImagen->bind_param("ss", $matricula, $ruta);
        //         if (!$insertarImagen->execute()) {
        //             die('Error al insertar imagen: ' . $insertarImagen->error);
        //         }
        //     }
        // }
    }
