...
<body>
    ...
    if (isset($_GET['matricula'])) {
        ...
        $consulta_imagen_coche = $_conexion->prepare("SELECT * FROM imagen_coche WHERE id_coche = ?");
        $consulta_imagen_coche->bind_param("s", $matricula_obtenida);
        $consulta_imagen_coche->execute();
        $datos_imagen_coche = $consulta_imagen_coche->get_result();

        while ($fila = $datos_imagen_coche->fetch_assoc()) {
            echo '<div class="col-md-4 text-center mb-3">';
            echo '<img src="' . htmlspecialchars($fila['ruta_img_coche']) . '" class="img-fluid rounded shadow" alt="Imagen del coche">';
            echo '<br><small>Imagen ID: ' . $fila['id_imagen_coche'] . '</small>';
            echo '</div>';
        }
    }
    ...
    <form action="#" method="post" enctype="multipart/form-data">
        ...
        <div class="row mt-4">
            ...
            while ($fila = $datos_imagen_coche->fetch_assoc()) {
                ...
                echo '<img src="' . htmlspecialchars($fila['ruta_img_coche']) . '" class="img-fluid rounded shadow mb-2" style="max-height: 200px;">';
                echo '<form action="reemplazar_imagen.php" method="POST" enctype="multipart/form-data">';
                echo '<input type="hidden" name="id_imagen_coche" value="' . $fila['id_imagen_coche'] . '">';
                echo '<input type="file" name="img_nueva" class="form-control mb-2" accept="image/*">';
                echo '<button type="submit" class="btn btn-warning btn-sm">Reemplazar</button>';
                echo '</form>';
                echo '</div>';
            }
            ?>
